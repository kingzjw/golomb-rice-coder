...
// 从bitset中取一个字节
uint8_t ReadBuffer::getByte()
{
	uint8_t b = static_cast<uint8_t>(bitBuffer.to_ulong());
	count -= 8;
	bitPos -= 8;
	bitBuffer >>= 8;
	return b;
...
// 向bitset中写入一个bit
void ReadBuffer::putBit(bool b)
{
	bitBuffer.set(bitPos, b);
	bitPos++;
	count++;
}

// 向bitset中写入一个byte
void ReadBuffer::putByte(uint8_t b)
{
	/*data <<= 8;
	data |= b;
	pos += 8;
...
BufferState BitInputStream::check()
{
	...
	if (count >= 64)
	{
		...
		if (bits.count >= 64)
		{
			if (bits.bitBuffer.none()) // 64 bits 0
